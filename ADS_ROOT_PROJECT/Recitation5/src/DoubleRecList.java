...
public class DoubleRecList {
    ...
    public void reverse() {
        rec_rev(front.getNextNode());

        front.getPrevNode().setPrevNode(front.getPrevNode().getNextNode());
        front.getPrevNode().setNextNode(front);

        front = front.getNextNode();
    }

    private void rec_rev(Node<String> curr) {
        if (front == null || front.getNextNode() == front.getPrevNode())
            return;

        if (curr.getNextNode() != front)
            rec_rev(curr.getNextNode());

        curr.getNextNode().setPrevNode(curr.getNextNode().getNextNode());
