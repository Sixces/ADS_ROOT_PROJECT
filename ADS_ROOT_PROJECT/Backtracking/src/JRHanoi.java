...
// There are two ways to use this program.  One is to actually "play the game"
// where you manually move the disks from tower to tower.  To play in this way
// you first select the tower from which you want to remove a disk (always the
// top disk on the tower) and then select the tower on which you want to place
// the disk.  Your goal is always to move all of the disks from Tower 0 to
// Tower 2.  Try it a few times with different numbers of disks.

// The second way to use this program is to let the program make all of the
// moves for you.  Try this as well to see how the solution progresses.
...
public class JRHanoi extends JFrame implements Runnable
{
    ...
    public void solveHanoi(int sz, int start, int middle, int end)
    {
        if (sz == 1)	// If we have only one disk, move it from start
        {					// to end directly
        ...
        {
            solveHanoi(sz-1, start, end, middle);
            theTowers[start].move(theTowers[end]);
            moves++;
            solveHanoi(sz-1, middle, start, end);
        }
    }
